...
]) or customErrorHandler(E_USER_ERROR, "Неверное ФИО");
$email = filter_input(INPUT_POST, 'email', FILTER_VALIDATE_EMAIL) or customErrorHandler(E_USER_ERROR, "Неверный email");
$password = $_POST['password'];
if (!$password) {
    customErrorHandler(E_USER_ERROR, "Пароль не может быть пустым");
}
$password_hash = password_hash($password, PASSWORD_DEFAULT);

$password_hash = password_hash(htmlspecialchars($_POST["password"]), PASSWORD_DEFAULT);
$qualification = filter_input(INPUT_POST, 'qualification', FILTER_VALIDATE_INT) or customErrorHandler(E_USER_ERROR, "Неверная специальность");
$image_fieldname = "avatar";
if ($_FILES[$image_fieldname]['error'] !== 0) {
    customErrorHandler(E_USER_ERROR, "Ошибка загрузки изображения");
}

if (!is_uploaded_file($_FILES[$image_fieldname]['tmp_name'])) {
    customErrorHandler(E_USER_ERROR, "Изображение не загружено корректно");
}

if (!getimagesize($_FILES[$image_fieldname]['tmp_name'])) {
    customErrorHandler(E_USER_ERROR, "Загруженный файл не является изображением");
}
...
if (db_query($DB, "INSERT INTO user (user_fullname, user_email, user_password, id_qualification) VALUES (?, ?, ?, ?);", [$fullname, $email, $password_hash, $qualification], 'sssi')) {
    ...
    if (!move_uploaded_file($_FILES[$image_fieldname]['tmp_name'], $avatar_filename)) {
        customErrorHandler(E_USER_ERROR, "Ошибка перемещения файла изображения");
    }
